...
def rank_players_by_war_utility(clan_tag: str, wars: int) -> list[WarPlayerStats]:
    ...
    current_members = {encode_tag(member["tag"]) for member in client.get_clan_members(clan_tag)}
    ...
    for race in races:
        ...
        for participant in clan_entry.get("participants", []):
            ...
            player_data[tag]["records"].append(
                {"fame": participant.get("fame", 0), "decks_used": decks_used}
            )
    ...
    results.sort(key=lambda player: player.war_utility, reverse=True)
    return results
